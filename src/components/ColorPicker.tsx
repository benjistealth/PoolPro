              ...
              {(pickerStyle === 'balls' || pickerStyle === 'backdrop') && colors.find(c => c.value.toLowerCase() === value.toLowerCase()) ? (
                <div className={`${pickerStyle === 'backdrop' ? 'w-[18vw] sm:w-[14vw]' : 'w-16 sm:w-20'} h-[6vh] sm:h-[8vh] rounded-lg overflow-hidden border border-white/10 shadow-lg flex items-center justify-center bg-slate-800`}>
                  {(() => {
                    ...
                    if (!imgSrc) return <div className="text-[10px] text-slate-500 font-bold">{selectedItem?.name}</div>;
                    return (
                      <img 
                        src={imgSrc} 
                        alt="Selected"
                        ...
                        onError={(e) => {
                          ...
                          fallback.className = 'text-[10px] text-slate-400 font-bold uppercase p-1 text-center';
                         ...
                         { (b.thumbnail || b.image) ? (
                           <img 
                             ...
                             onError={(e) => {
                               ...
                               const fallback = document.createElement('div');
                               fallback.className = 'text-[10px] text-slate-400 font-bold uppercase p-2 text-center';
